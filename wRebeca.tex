...
\fixme{what do you mean: "as their nodes ..."? in wired networks, nodes do not operate in a completely collaborative and distributed manner? }
...
\fixme{You sure you want to use (dis)connectivity?????}
...
\Marjan{Because the topology is changing all the time, finding a scenario that leads to protocol malfunction (for example  formation of a loop in the routing protocol), is very unlikely if we use  simulation-based approaches. So, these approaches are not very helpful in designing such protocols in practice.  }
...
\fixme{you said the topology is at the level of the transition system, then how this succ and unsucc are handled?}
...
is shown partially in Figure \ref{Fig::stbefore}. The message handling transitions are represented by $a/b$-transitions while random modification of the topology is shown by $\tau$-transitions. %$\tau$-transitions show the random changes of the topology while $a/b-$transitions denote handling of message queues
%For generating the first type, the effect of communication statements like broadcast is computed with respect to the topology.
\fixme{is the following sentence what you wanted to say? but then you are saying that the topology is not changed, what about the second type of transition? Ok, you said that later.}
...
\Marjan{level} and topology elimination at the \Marjan{level of the transition system.} 
%semantics. 
Assume a snapshot of a model 
\Marjan{where we have}
%that there exists 
...
\fixme{why you have k actors, and also k statements?}
...
\Marjan{, queue contents, and the pointer to the statements to be executed (PC).}
...
\Marjan{Table \ref{Tab:aodv-redu} shows ...}
