...
\Marjan{where $S$ denotes the local states of rebecs and $\gamma_i$ denotes the  topology}
\fixme{where $S$ and $\gamma_i$ denote the local states of rebecs and topology respectively}, are aggregated as demonstrated in Figure \ref{Fig::stafter}. %A transition is defined from $S$ to $S_1$ if there exists $\gamma_1$ such that $(S,\gamma_1)\overto (S_1,\gamma_1)$. 
...
\fixme{I think it is better to have $S_0$ instead of S.}

that results the same modification on the local states of actors %in the original model 
...
\fixme{Using (dis-)connectivity is very confusing, why not just saying connectivity? or connectivity/disconnectivity}

For instance, ${\it and}({\it con}(n_1,n_2),!{\it con}(n_3,n_4)$ denotes that the actor  $n_1$ is connected to $n_2$ while $n_3$ is disconnected from $n_4$. The network constraints on transitions express the conditions that the combined states have symmetric behaviors. \fixme{I dont understand the previous sentence} Our reduction technique is indeed a form of symmetric reduction technique \cite{clarke1998symmetry}. This form of reduction can be applied if the actors were not isolated. %; it is independent from the  
However, the isolation of actors makes this reduction more effective for the analysis of real-world protocols like AODV. 
\fixme{this is a very bad sentence because it doesnt convey any meaning}
...
\fixme{the above sentence can be written in a much more understandable way}
