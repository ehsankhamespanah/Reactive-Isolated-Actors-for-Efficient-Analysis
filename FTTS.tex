...
\begin{figure*}[!htbp]
 	\begin{center}
 		...
 		\Marjan{No, I dont want to have constructor explicitly in the grammar, I want just want an explanation in the caption that the constructor is a message server with the name of the class with two parameters?? one is the queue length ...}}
...
\Marjan{Note that there is no statement in Timed Rebeca that allows access to the absolute value of \textit{now}  or let the modeler to assign an absolute value to it.}
...
\begin{table*}
  \begin{center}
        ...
        \end{tabular}
        \end{center}
        \caption{Comparing the number of states and transitions in TTS and FTTS of three different example (from~\cite{ehsan-phd-thesis})\fixme{Ehsan, would you please add the reference?
        Do you mean $<1$ sec ?
        Dont we have larger examples where tts is impossible but ftts is possible?}}
